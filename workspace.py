...
def transformation_matrix(theta, L):
    T = np.array([[np.cos(theta), 0, np.sin(theta), L*np.sin(theta)],
                   [0, 1, 0, 0],
                   [-np.sin(theta), 0, np.cos(theta), L*np.cos(theta)],
                   [0, 0 , 0, 1]])
...
def compute_fk(joint_angles):
    ...
    return T_link2[0, 3], 0, T_link1[2, 3]
